...
2. **게임 결과 씬 제작 → 이탈 출구 연결 + 사망 연출** — 이탈 흐름(`BeginMatchExit` → 4초 → `CompleteMatchExit`)은 완성됐고 **씬 전환만 `TODO:`로 비어 있다.** 지금은 4초 뒤 연결만 끊고 인게임에 머물러 **테스트 종료가 강제 종료다.** 씬이 생기면 `CompleteMatchExit()`의 TODO를 전환으로 교체하고, 로비 재진입 분기(`LobbyScene.cs:87`) + 진입 직후 `GetInventoryCall()` 재조회를 붙인다(**`OnLoginComplete()`이 캐시 배열을 읽으므로 재조회 완료 후 배치하도록 순서를 고정할 것**). 사망 연출(탑뷰 카메라 전환)은 `BeginMatchExit()`의 TODO 자리 — 기존 카메라를 옮기지 않고 **새 카메라를 만들어 전환**하는 설계라 `ApplyViewRotation()`과 충돌하지 않는다
...
### 확인 필요
- **(2026-08-21) T18 검증** — 죽었을 때 ① 킬 피드 로그 뒤 `[MatchExit] 이탈 시작` ② 4초간 조작이 전부 막히고 **남들은 계속 움직이는지**(관전 유지) ③ `[MatchExit] 연결 종료` ④ 이후 인게임에 남는 것이 현재 의도된 동작(강제 종료로 끝낼 것)
