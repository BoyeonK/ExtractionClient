...
0. **`TestIngame.unity` → `TestIngameScene.unity` 리네임 (에디터 작업, 선행 필수)** — 코드는 이미 `Define.Scene.TestIngameScene`을 가리키므로 **리네임 전까지 매치 진입이 씬을 찾지 못한다.** 함께 Build Settings에 `GameResultScene`이 등재되어 있는지 확인할 것(미등재면 결과 씬 전환이 실패한다). `TestIngame2.unity`도 컨벤션을 따르려면 `TestIngame2Scene.unity`이지만 enum에 없어 코드 영향은 없고, `WinchesterAlpha.unity`는 맵 에셋 성격이라 판단 보류
1. **서버 변경분 반영 (`server-sync-todo.md`)** — T1~T11 완료(핸들러는 41번까지 전부 등록 완료 — proto의 `[작업사항]` 중 패킷 배선이 필요한 항목은 남은 것이 없다), T10은 데이터 경로가 끝나고 **UI 프리팹 대기**, T12는 **런타임 실측만 남음**, **T15·T18은 결과 씬 전환·로비 복귀까지 붙어 표시 UI만 남았다**. 남은 것은 T16·T17. **T13은 시신 프리팹 제작 대기(아래 2번), T14는 서버 목록 대기**. 전 항목 반영이 끝나면 proto의 `[작업사항]` 블록을 일괄 삭제할 것(proto 11~12행 지시) — 그 전에 지우면 잔여 작업 추적 근거가 사라진다
...
### 확인 필요
- **(2026-08-24) 세션 유지 경로 검증** — 로그인 → 매치 → 사망/귀환 → 4초 유예 → 결과 씬 자동 전환 → 결과 로그 → 엔터 → **Login UI 없이** 로비 진입 + `세션 유지 성공` 로그. 이어서 ① 서버 중지 또는 Redis 세션 삭제 상태로 반복 → 만료/불통 안내가 뜨고 TestStart 플로우로 합류하는지 ② 게스트 로그인으로 반복 → Header가 Guest로 복귀하는지 ③ **일반 최초 실행 경로(게임 시작 → 로비)가 기존과 동일한지 회귀 확인**. 선행 조건은 위 우선순위 0번(씬 리네임·Build Settings)
...
- **(2026-08-21) T18 검증** — 죽었을 때 ① 킬 피드 로그 뒤 `[MatchExit] 이탈 시작` ② **카메라가 떠오르며 시신을 내려다보는지** ③ 4초간 조작이 전부 막히고 **남들은 계속 움직이는지**(관전 유지) ④ `[MatchExit] 연결 종료` ⑤ **다음 프레임에 결과 씬으로 넘어가는지**((2026-08-24 #0)에서 전환이 붙어 더 이상 인게임에 남지 않는다)
