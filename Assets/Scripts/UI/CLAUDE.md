...
- **플레이어 위치는 UI 좌표 환산이 아니라 월드 마커로 표현한다** — `PlayerObject` 자식의 `MapPoint/BluePoint`(레이어 `ShownOnlyOnMap`)가 탑뷰 컬링 마스크에 걸려 **이미지 안에 함께 찍힌다.** 그래서 UI 쪽에는 위치 계산이 없고, **`OppoPlayerObject`에 같은 마커를 붙이면 적 위치가 그대로 보인다**
  - **마커에 콜라이더를 붙이지 말 것** — `ICombatTarget` 탐색이 `GetComponentInParent`라 플레이어 하위에 붙은 콜라이더는 무엇이든 피격 부위가 된다
  - **`Cast Shadows`는 꺼져 있고 되돌리지 말 것** — 게임 화면에서 그림자가 관측된 적은 없지만 **카메라 컬링 마스크가 그림자까지 보장하지는 않아** 선제로 막은 것이다. 되살아나면 증상이 "플레이어 주변의 정체불명 원형 그림자"라 원인을 짚기 어렵다
  - 코드는 이 두 이름을 찾지 않는다 — `SoundPoint`·`MuzzlePoint`처럼 **이름이 계약인 자리와 갈린다**
...
- **방위 라벨(N/E/W/S)은 눈금 각도가 아니라 `deg - NORTH_DEG`(180)를 감은 값으로 정한다** — 그래서 180도 자리가 N, 270이 E, 0·360이 S, 90이 W다. **감지 않으면 복제 구간이 라벨을 잃는다**(`-60`·`420`은 어느 방위에도 걸리지 않는다). 큰/작은 눈금 판정만은 360이 30의 배수라 원값으로도 결과가 같다
- **라벨은 해당 눈금의 자식이다** — x를 눈금이 끌고 가므로 `LayoutTicks`가 라벨을 따로 밀지 않는다. `Strips` 직계로 올리면 위치를 갱신할 곳이 하나 늘고 해상도가 바뀐 프레임에 라벨만 제자리에 남는다
- **눈금 세로 위치는 `TICK_OFFSET_Y`(-6)이고 라벨은 그 위에 얹힌다**(`MAJOR_HEIGHT/2 + LABEL_GAP + LABEL_HEIGHT/2`, 눈금 기준 로컬). `Strips` 자체의 y는 여전히 저작값이다 — 세로를 옮길 때 어느 쪽을 만질지 헷갈리면 코드는 이 상수 하나만 본다
