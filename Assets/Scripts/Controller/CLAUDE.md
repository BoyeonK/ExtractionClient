...
- **`Increase`/`DecreaseAggro`는 `SetAggro`로 수렴한다** — 주체 가드와 클램프가 각각 한 자리여야 갈래가 늘어도 빠지지 않는다. **클램프 결과가 그대로면 보내지 않는다** — 46번이 reliable이라 헛 전송이 in-flight 32슬롯을 먹는다
- **`GetAggro`만 주체 가드 밖이다**(주체가 아닐 때 부르는 것이 존재 이유). **이름이 '획득'이지 접근자가 아니다** — 값 읽기는 `Aggro` 프로퍼티
- **피격으로 생기는 aggro는 서버가 정한다 — `ProcessHit`에 `GetAggro`를 붙이지 말 것**(서버 계약). 클라가 피격마다 요청하면 판정이 두 곳이 되고, 요청값이 MAX면 첫 한 발로 주도권이 굳어 아무도 뺏을 수 없다. 반영 경로는 통보(49) → `ApplyServerAuthority` 하나다
- **그래도 `GetAggro`는 죽은 코드가 아니다** — 피격 **외의** 적대 행동(근접·소음 등)은 여전히 클라가 요청한다. 지금 호출부가 없는 것은 그 계기가 아직 설계되지 않았기 때문이며, 미사용이라고 걷지 말 것
- **`MaxAggro`에 닿으면 주도권이 굳는다 — 의도이고 서버도 같게 판정한다**(47번 계약). 비교식을 `>=`로 완화하지 말 것
...
#### 카메라 위치는 앵커가 쥔다 (`UpdateCameraAnchor`)

`Pcam`은 스스로 위치를 갖지 않고 `_isLeftAngleCam`이 가리키는 앵커(`ViewPoint/RightCamPoint` · `LeftCamPoint`)의 `localPosition`을 매 프레임 받는다. **회전은 부모 `ViewPoint`가, 위치는 여기가 쥐어 축이 갈리므로 `ApplyViewRotation()`과 순서를 다투지 않는다.**

- **프리팹에서 `Pcam`을 옮겨도 반영되지 않는다 — 조정 대상은 두 앵커다.** 증상이 "에디터에서 옮겼는데 게임에선 그대로"라 원인을 짚기 어렵다
- **앵커는 `ViewPoint`의 자식이라 루트에서 `transform.Find`하면 null이다.** 못 찾으면 `LogError` 후 저작 위치에 고정된다 — `ShotPoint`와 달리 대체할 것이 없다
- **`ProcessAim`·`ProcessFire`보다 앞에서 돌아야 한다** — 카메라 위치에서 레이가 나가므로, 뒤에 두면 시점을 바꾼 프레임의 조준이 직전 위치 기준으로 나간다
- **rotation은 따라가지 않는다 — 기울이기(린/피킹)는 구현 예정이 없다**(확정). 앵커 둘 다 회전이 identity이고 x만 ±0.2인 **좌우 평행 이동**이다. `ChangeCamAngle`이라는 이름은 게이머 용어 '좌각/우각'을 옮긴 것이지 roll을 뜻하지 않는다
- **보간은 즉시 대입이 아니다**(`CAM_ANGLE_MOVE_SPEED`, 반동과 같은 형태·같은 값 15) — PlayerObject 스케일이 2라 로컬 0.4가 월드 0.8이고, 대입하면 어깨 너비만큼을 한 프레임에 건너뛴다
- **전환은 `ChangeCamAngle()` 하나이고 중앙 상태는 없다**(확정). `IsInputLocked`·`IsAnyUIOpen`에서 막는다 — 전자를 빼면 귀환·연결 끊김 이탈 중(연출이 없어 원래 카메라가 남는다) 화면이 옆으로 튀고, 후자를 빼면 창을 띄운 채 뒤 화면이 움직인다
- **키는 `Q`이고 등록은 `PlayerController.Init()`, 해제는 `OnDestroy()`다 — 바꿀 때는 한 쌍으로 바꿀 것**(한쪽만 고치면 해제가 빗나가 리스너가 남는다). 좌우 피킹의 표준이 Q·E인데 **E는 상호작용이 점유**하고 있어 Q 단독 토글이다
- **옮기는 것은 카메라뿐이다 — 모델·히트박스·`_shotPoint` 어느 것도 따라가지 않으며 이것이 설계다.** `_shotPoint`를 카메라에 맞추려 들지 말 것(아래 '발사선'): 크로스헤어는 카메라에서 나와 모서리 너머 적에 걸리는데 발사선 원점은 가슴팍이라 엄폐물에 막힌다 — **보이지만 못 쏘는 것이 정상이고**, 그래서 이 기능은 시야 도구일 뿐 peeker's advantage를 만들지 않는다
- **내 몸이 움직이지 않으므로 '화면에선 피했는데 맞는다'는 생기지 않는다.** 서버에 자세를 알릴 필요가 없는 근거이기도 하다(proto에 필드가 없는 것이 누락이 아니다)
