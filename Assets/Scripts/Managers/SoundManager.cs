...
public class SoundManager {
    ...
    //
    // 가청 거리는 소스가 정한다 — 호출부가 카테고리에 맞는 소스를 골라 넘긴다(근거리 SoundPoint /
    // 총성 GunSoundPoint). MaxDistance·Rolloff는 소스 단위 속성이라 이 함수에서 재생마다
    // 갈아끼우면 울리고 있던 소리의 거리감이 중간에 튄다(걸으며 쏘는 상황이 상시라 실제로 겹친다).
    // 거리 인자를 받는 오버로드를 만들지 말 것
