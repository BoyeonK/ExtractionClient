...
- 맵 씬 클래스는 `SceneType` 대입 한 줄만 다르고 나머지는 `IngameScene`이 전부 한다 — **맵별 분기를 씬 클래스에 넣기 전에 서버 주도인지부터 볼 것**(정적 오브젝트는 블루프린트로 내려온다)
  - **분기를 넣기 전에 루트에 붙은 스크립트도 확인할 것** — 다른 맵 씬을 복제해 만들었으면 옛 씬 스크립트가 남아 있어도 증상이 없다(`SceneType`은 읽는 코드가 한 곳도 없고 인게임 배선이 전부 `is IngameScene`으로 걸린다). **분기를 넣는 순간에야 "안 먹는다"로 드러난다**
...
- 미스폰 NPC의 통보에는 `RequestSpawnIfUnknown()`만 걸고 **보류 목록을 두지 않는다**. 스폰 뒤 주도권을 다시 알려주는 것은 서버 몫이다
- **상태 송신(`BuildMyNpcStates`)은 내가 주도하는 것만 싣고, 없으면 `null`을 돌려준다** — 매 틱 도는 경로라 빈 목록을 만들지 않는다. 버퍼를 재사용해도 안전한 것은 `SendUnreliable`이 호출 즉시 직렬화하기 때문이다
- **수신(`UpdateNpcStates`)은 모르는 objectId에 스폰을 요청하지 않는다** — 10Hz라 reliable이 폭주한다(발사 브로드캐스트와 같은 판단). 채우는 것은 주도권 통보 쪽이다
...
**세는 대상은 컨테이너·내 인벤토리·설정 창·ESC 창·지도 다섯이고 각자 자기 플래그로 짝을 맞춘다** — **카운트가 0으로 안 떨어지면 그 판 내내 커서가 풀린 채 총이 안 나간다.** 반대로 새 UI를 세면서 빠뜨리면 **커서가 잠긴 채라 그 UI의 버튼을 클릭할 수 없다.**
...
- `BeginMatchExit()`이 다섯을 모두 정리한다. 설정 창은 `CancelAndHide()`가 아니라 **`Hide()`** — 이미 반영된 값을 되돌릴 이유가 없다. **새 UI를 여기 빠뜨리면 사망 연출 위에 남고 카운트도 0으로 안 떨어진다**

### 인게임 키 라우팅

| 키 | 동작 |
|---|---|
| `Tab` / `I` | `ToggleMyInventory()` — **컨테이너 상호작용 중이면 그 종료가 우선**(확정), 아니면 인벤토리 토글. 설정 창·ESC 창·지도가 떠 있으면 무시 |
| `Esc` | `OnEscapeInput()` — **가장 위에 있는 것부터 닫는다**(설정 → ESC 창 → 지도 → 컨테이너 → 인벤토리). 닫을 것이 없을 때만 **ESC 창**을 연다 |
| `M` | `ToggleMapView()` — 지도 토글. **다른 창이 하나라도 떠 있으면 무시**(`IsAnyUIOpen`)라 지도는 어떤 UI와도 겹치지 않는다 |
| `E` | `TryInteract()` |
