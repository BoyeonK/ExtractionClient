...
public class IngameScene : BaseScene {
    ...
    // 유예 시간. WeaponSpec에 재장전·교체 시간 필드가 없어 DB에서 오지 않는 클라 상수다.
    // 두 값은 2026-08-27 플레이 검증에서 손에 맞는 것이 확인된 확정치다 —
    // TODO: 애니메이션이 붙으면 클립을 이 길이에 맞춰 만들 것. 반대로 클립 길이에 상수를
    //       맞추면 확정된 조작 감각이 조용히 바뀐다. 무기별로 달라져야 하면 ItemDBHelper를
    //       고치지 말고(수정 금지) DB 쪽에 필드 추가를 요청할 것
