...
public class PacketHandler {
    ...
    private void Handle_D2CResponseSpawnMeSpawnSpot(ReadOnlySpan<byte> payloadSpan) {
        ...
        Managers.ExecuteAtMainThread(() => {
            ...
            if (scene is IngameScene ingameScene) {
                ...
                ingameScene.HandleSpawnSpot(spawnPoint, pkt.CharacterType, pkt.ObjectId, pkt.RemainingLifeMs);
            }
        });
    }
    ...
    private void Handle_D2CResponseRecentContainerInfo(ReadOnlySpan<byte> payloadSpan) {
        D2CResponseRecentContainerInfo pkt = null;

        try {
            pkt = D2CResponseRecentContainerInfo.Parser.ParseFrom(payloadSpan);
        }
        catch (InvalidProtocolBufferException e) {
            Managers.ExecuteAtMainThread(() => { Util.LogError($"D2CResponseRecentContainerInfo 파싱 실패: {e.Message}"); });
            return;
        }
        catch (Exception e) {
            Managers.ExecuteAtMainThread(() => { Util.LogError($"D2CResponseRecentContainerInfo 처리 중 알 수 없는 에러: {e.Message}"); });
            return;
        }
    ...
    private void Handle_D2CResponseRecall(ReadOnlySpan<byte> payloadSpan) {
        ...
        Managers.ExecuteAtMainThread(() => {
            if (Managers.Scene.CurrentScene is not IngameScene ingameScene) return;
            ingameScene.HandleRecallResponse(result, recallSpotIndex);
        });
    }

    private void Handle_D2CNotifyRecallResult(ReadOnlySpan<byte> payloadSpan) {
        D2CNotifyRecallResult pkt = null;

        try {
            pkt = D2CNotifyRecallResult.Parser.ParseFrom(payloadSpan);
        }
        catch (InvalidProtocolBufferException e) {
            Managers.ExecuteAtMainThread(() => { Util.LogError($"D2CNotifyRecallResult 파싱 실패: {e.Message}"); });
            return;
        }
        catch (Exception e) {
            Managers.ExecuteAtMainThread(() => { Util.LogError($"D2CNotifyRecallResult 처리 중 알 수 없는 에러: {e.Message}"); });
            return;
        }
    ...
    private void Handle_D2CNotifyWeaponChanged(ReadOnlySpan<byte> payloadSpan) {
        D2CNotifyWeaponChanged pkt = null;

        try {
            pkt = D2CNotifyWeaponChanged.Parser.ParseFrom(payloadSpan);
        }
        catch (InvalidProtocolBufferException e) {
            Managers.ExecuteAtMainThread(() => { Util.LogError($"D2CNotifyWeaponChanged 파싱 실패: {e.Message}"); });
            return;
        }
        catch (Exception e) {
            Managers.ExecuteAtMainThread(() => { Util.LogError($"D2CNotifyWeaponChanged 처리 중 알 수 없는 에러: {e.Message}"); });
            return;
        }
    ...
    private void Handle_D2CNotifyObjectKilled(ReadOnlySpan<byte> payloadSpan) {
        D2CNotifyObjectKilled pkt = null;

        try {
            pkt = D2CNotifyObjectKilled.Parser.ParseFrom(payloadSpan);
        }
        catch (InvalidProtocolBufferException e) {
            Managers.ExecuteAtMainThread(() => { Util.LogError($"D2CNotifyObjectKilled 파싱 실패: {e.Message}"); });
            return;
        }
        catch (Exception e) {
            Managers.ExecuteAtMainThread(() => { Util.LogError($"D2CNotifyObjectKilled 처리 중 알 수 없는 에러: {e.Message}"); });
            return;
        }
